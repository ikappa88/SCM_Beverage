...
def create_initial_users():
    ...
    try:
        ...
        tc_tokyo_id = str(tc_tokyo.id) if tc_tokyo else "5"
        tc_osaka_id = str(tc_osaka.id) if tc_osaka else "6"
        ...
        print("初期ユーザーを作成しました：")
        print(f"  管理者:  username=admin        password=admin1234")
        print(
            f"  実務者1: username=operator1    password=operator1234  拠点ID={tc_tokyo_id}"
        )
        print(
            f"  実務者2: username=operator2    password=operator1234  拠点ID={tc_osaka_id}"
        )
