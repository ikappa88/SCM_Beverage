    ...
    <AdminLayout>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">全拠点の発注・配送スケジュール（参照のみ）</p>
        </div>
                  ...
                  {cols.map((col) => {
                    ...
                    return (
                      <td key={col.label} className={`py-2.5 px-3 text-center ${hasActivity ? "bg-teal-950/30" : ""}`}>
                        {orderQty > 0 && (
                          <div className="text-xs text-blue-400 leading-tight">発注 {orderQty.toLocaleString()}</div>
                        )}
                        {deliveryQty > 0 && (
                          <div className="text-xs text-teal-400 leading-tight">配送 {deliveryQty.toLocaleString()}</div>
                        )}
