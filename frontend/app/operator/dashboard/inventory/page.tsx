...
export default function InventoryPage() {
  ...
  const filtered = inventories.filter((inv) =>
    ...
    inv.location.name.includes(search) ||
    inv.product.category.includes(search)
  );
    ...
    <OperatorLayout>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">担当拠点の在庫状況と実棚修正</p>
        </div>
        <input
          value={search}
                ...
                <tr key={inv.id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                  ...
                  <td className="py-2.5 px-4 text-right">
                    ...
                    <div className="w-16 h-1 bg-gray-800 rounded-full overflow-hidden ml-auto mt-1">
                      <div className={`h-full rounded-full ${inv.quantity < inv.safety_stock ? "bg-red-500" : "bg-teal-500"}`}
                        style={{ width: `${pct}%` }} />
                    </div>
                  </td>
      ...
      {editTarget && !showConfirm && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-full max-w-md">
            <h3 className="text-base font-semibold mb-1">実棚修正</h3>
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 w-full max-w-md">
            ...
            <div className="bg-gray-800 rounded-lg p-4 space-y-2 mb-4 text-sm">
              <div className="flex justify-between">
                ...
                <span>{editTarget.location.name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400">商品</span>
                <span>{editTarget.product.name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400">修正前</span>
                <span className="text-gray-400">{editTarget.quantity.toLocaleString()}</span>
              </div>
              <div className="flex justify-between font-medium">
                <span className="text-gray-400">修正後</span>
                <span className="text-teal-400">{Number(editQty).toLocaleString()}</span>
              </div>
              {editNote && (
                <div className="flex justify-between">
                  <span className="text-gray-400">備考</span>
                  <span className="text-xs">{editNote}</span>
                </div>
              )}
            </div>
